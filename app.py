...
def administrador():
    ...
    cursor.execute("""
        ...
        usuario, accion, modulo, ip
        FROM auditoria ORDER BY fecha DESC LIMIT 10
    """)
    actividad = cursor.fetchall()
    notificaciones = []
    if pendientes > 0:
        notificaciones.append({"tipo": "warning", "mensaje": f"⚠️ Existen {pendientes} solicitudes pendientes por revisar."})
    if total_solicitudes == 0:
        notificaciones.append({"tipo": "info", "mensaje": "ℹ️ No existen solicitudes registradas en el sistema."})
    else:
        notificaciones.append({"tipo": "success", "mensaje": "✅ El sistema funciona correctamente."})
    total_usuarios = total_estudiantes + total_docentes + total_administradores
    notificaciones.append({"tipo": "primary", "mensaje": f"👥 Usuarios registrados en el sistema: {total_usuarios}"})
    cursor.close()
    conexion.close()
    return render_template("panel_administrador.html",
        estudiantes=total_estudiantes, docentes=total_docentes,
        administradores=total_administradores, solicitudes=total_solicitudes,
        pendientes=pendientes, aprobadas=aprobadas, rechazadas=rechazadas,
        actividad=actividad, notificaciones=notificaciones)
...
def editar_beneficio(id):
    ...
    if request.method == "POST":
        ...
        if nombre_beneficio != beneficio_anterior["nombre_beneficio"]:
            registrar_detalle_auditoria(id_auditoria, "nombre_beneficio", beneficio_anterior["nombre_beneficio"], nombre_beneficio)
        if descripcion != beneficio_anterior["descripcion"]:
            registrar_detalle_auditoria(id_auditoria, "descripcion", beneficio_anterior["descripcion"], descripcion)
        if estado != beneficio_anterior["estado"]:
            registrar_detalle_auditoria(id_auditoria, "estado", beneficio_anterior["estado"], estado)
        cursor.execute("""
            UPDATE beneficios
...
def auditoria():
    ...
    return render_template("auditoria.html", auditorias=auditorias,
        total_acciones=total_acciones, total_logins=total_logins,
        total_ediciones=total_ediciones, total_eliminaciones=total_eliminaciones,
        usuario_activo=usuario_activo, modulo_principal=modulo_principal,
        accesos_roles=accesos_roles, actividad_diaria=actividad_diaria,
        acciones_modulos=acciones_modulos, usuarios_activos=usuarios_activos,
        acciones_tipo=acciones_tipo, buscar=buscar, rol=rol, modulo=modulo,
...
def limpiar_historiales():
    ...
    cursor.close()
    conexion.close()
    return render_template("limpiar_historiales.html", total_auditoria=total_auditoria, total_detalle=total_detalle)
